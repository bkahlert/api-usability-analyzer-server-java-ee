package de.fu_berlin.imp.apiua.server.java_ee.utils.testsite;
	...
	public void handle(Request request, Response response) {
		try {
			...
			if (absPath != null) {
				...
				/*
				 * make APIUAclt include code point to the correct server; since
				 * we want to use protocol-relative URLs we remove an eventually
				 * specified protocol
				 */
				URI[] APIUAsrvUris = TestConfiguration
						.getSchemaRelativeAPIUAsrvURIs();
				Assert.assertTrue(
						"You must at least configure one APIUAsrv to be tested",
						APIUAsrvUris.length > 0);
				if (APIUAsrvUris.length > 1) {
					LOGGER.fatal("There is more than one APIUAsrv to be tested. Only testing the first the following APIUAsrvs:\n"
							+ StringUtils.join(APIUAsrvUris, "\n"));
				}
				content = content.replace("${APIUAsrv.url}",
						APIUAsrvUris[0].toString());
