 ...
 * and GPL (GPL-LICENSE.txt) licenses.
 * 
 * 1) Some page (example.com) includes http(s)://dalak.imp.fu-berlin.de/SUAsrv/static/js/SUAclt.js (this script)
 * 2) The page finished loading
 ...
 *    3) SUAsrv.html sets its window.name to "SUAsrv-[last fingerprint]-[new fingerprint]
 *    4) SUAsrv.html redirects back to example.com (which is the query portion in 3.1) and the query ?noSUA
 *       - redirecting to the calling web site makes window.name accessible by it
 *       - ?noSUA disables data collection in order to avoid infinitive recursion
 *    5) SUAclt.js receives the load event and extracts the last and current fingerprint from the iframe's window.name
 * 4.a) If the two fingerprints differ, associateNewFingerprint is called and the logging is activated with a
 *      small delay. (SUAsrv does not correctly handle immediate log events after a fingerprint change.)
 * 4.b) If the fingerprint has not changed the logging is immediately activated.
 *     
 **/
(function($) {
	function associateNewFingerprint(lastFingerprint, currentFingerprint) {
		$.ajax({
			url: ('https:' == document.location.protocol ? 'https://' : 'http://') + "${HOST}${CONTEXT_PATH}/rest/fingerprint/!" + lastFingerprint + "/associate/!" + currentFingerprint,
			type: "GET",
			dataType: "script"
		});
	}
	...
	function activateLogging(fingerprint) {
		...
		$("input").keydown(function(e) {
			...
			log(fingerprint, "typing-" + id + "-" + $(this).val());
		});
	...
	$(document).ready(function() {
		if(window.location.search == "?noSUA") return;
		...
		iframe.attr("src", (("${HOST}".indexOf(":") >= 0) ? "http://" : "https://") + "${HOST}${CONTEXT_PATH}/static/SUAsrv.html?" + encodeURI(returnURI));
