 ...
 * http(s)://dalak.imp.fu-berlin.de/APIUAsrv/static/js/APIUAclt.js (this script) 2)
 * The page finished loading 3) APIUAclt.js checks if the browser's fingerprint
 * has changed. Due to the same origin policy (example.com !=
 * dalak.imp.fu-berlin.de) APIUAclt.js can't access a locally saved old
 ...
 * 1) APIUAclt.js inserts an invisible iframe and points to
 * http(s)://dalak.imp.fu-berlin.de/APIUAsrv/static/APIUAsrv.html?example.com 2) In
 * the context of http(s)://dalak.imp.fu-berlin.de APIUAsrv.html saves the current
 ...
 * called and the logging is activated with a small delay. (APIUAsrv does not
...
(function($) {
	...
	var APIUAsrvURL = (("${HOST}".indexOf(":") >= 0) ? "http://" : "https://") + "${HOST}${CONTEXT_PATH}";
	
	function associateNewFingerprint(lastFingerprint, currentFingerprint) {
		$.ajax({
			url: APIUAsrvURL + "/rest/fingerprint/!" + lastFingerprint + "/associate/!" + currentFingerprint,
			type: "GET",
			dataType: "jsonp"
		});
	}
	...
	$(document).ready(function() {
		...
		iframe.load(function() {
			...
			if(fingerprints.length == 3) {
				if(fingerprints[0] == "APIUAsrv" && fingerprints[1] != fingerprints[2]) {
